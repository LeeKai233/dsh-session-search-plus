		// [search-plus] Fork of @deepseek-ai/dsh-client-ui-workspace@0.1.0-rc.6
		// (baseline: the deployment's compiled lib/client.js). Unmodified regions
		// are byte-identical to the official bundle; every modification below is
		// marked with `// [search-plus]`.
		let _deepseek_ai_dsh_client_runtime_client = require("@deepseek-ai/dsh-client-runtime/client");
		let react_jsx_runtime = require("react/jsx-runtime");
		let react = require("react");
		let _deepseek_ai_dsh_client_ui_primitives = require("@deepseek-ai/dsh-client-ui-primitives");
		// [search-plus] private helpers: matcher/coloring + jump/highlight.
		let searchPlusMatcher = require("./matcher.ts");
		let searchPlusHighlight = require("./highlight.ts");
		// [search-plus] filter state rules (chips + active highlight).
		let searchPlusFilters = require("./filters.ts");
		// [search-plus] module-level jump state (the current mark target; the
		// MutationObserver in WorkspaceBrowser re-marks after React re-renders).
		let jumpListeners = [];
		let jumpState = { target: null };
		const getJumpState = () => jumpState;
		const setJumpState = (next) => {
			jumpState = next;
			for (const listener of jumpListeners) listener();
		};
		const subscribeJump = (listener) => {
			jumpListeners.push(listener);
			return () => {
...
`;
		const ensureSearchPlusCss = () => {
			if (typeof document !== "undefined" && searchPlusCssTag === null) {
				searchPlusCssTag = document.createElement("style");
				searchPlusCssTag.dataset.plugin = "dsh-session-search-plus";
				searchPlusCssTag.textContent = searchPlusCss;
				document.head.appendChild(searchPlusCssTag);
			}
		};
		// [search-plus] dictionaries moved to ./locales.ts (paradigm copy layer).
		let searchPlusLocales = require("./locales.ts");
		const searchPlusZh = searchPlusLocales.searchPlusZh;
		const searchPlusEn = searchPlusLocales.searchPlusEn;
		...
		*/
		// [search-plus] deriveSearchResults extended: scope/fuzzy/case-aware
		// title matching, colored runs, per-session hit lists for the dropdown.
		function deriveSearchResults(list, workspaces, query, archivedSessionIds, content, limit, filters) {
			const q = query.trim();
			if (q === "") return {
				items: [],
				hasMore: false
			...
			const local = [];
			// [search-plus] title runs live in a per-call map: never mutate the
			...
			return {
				items: ordered.slice(0, limit).map((summary) => {
					...
					const titleRuns = titleRunsById.get(summary.id);
					return {
						id: summary.id,
						...
						// [search-plus] richer content payload for coloring/dropdown/jump.
						...match === void 0 ? {} : { snippet: match.snippet },
						...match === void 0 ? {} : { matchRuns: match.matchRuns },
						...match === void 0 ? {} : { matches: match.matches },
						...titleRuns === void 0 ? {} : { titleRuns }
					};
				}),
		...
		*/
		...
		function SearchResultItem({ result, currentId, onOpen, onJump, expanded, onToggleExpand, filters, query, t }) {
			...
			const colored = (text, runs) => {
				if (runs === void 0 || runs === null || runs.length === 0) return text;
				return searchPlusMatcher.segmentsOf(text, runs).map((segment, index) => segment.hit
					? (0, react_jsx_runtime.jsx)("span", { key: index, className: "dsh-sp-hit", children: segment.text })
					: (0, react_jsx_runtime.jsx)("span", { key: index, children: segment.text }));
			};
			...
			const openBest = () => {
				if (best !== void 0) jumpWith(best);
				else onOpen(result.id);
			};
			// [search-plus] one visual block: the official row class supplies the
			// background/hover/selected for title area AND chevron together; the
			// chevron is a plain glyph button inside the same block (row width =
			// search box width, nothing spills).
			return (0, react_jsx_runtime.jsxs)("div", {
				className: "dsh-sp-rowWrap",
				children: [
					(0, react_jsx_runtime.jsxs)("div", {
						className: clsx(Rows_module_css_default.searchResultRow, "dsh-sp-rowFrame", selected && Rows_module_css_default.selected),
						role: "treeitem",
						...
						children: [(0, react_jsx_runtime.jsxs)("div", {
							...
							children: [(0, react_jsx_runtime.jsxs)("span", {
								...
								children: [(0, react_jsx_runtime.jsx)("span", {
									...
									children: colored(result.title, result.titleRuns)
								})]
							}), (0, react_jsx_runtime.jsxs)("span", {
								...
								}), result.snippet !== void 0 && !expanded && (0, react_jsx_runtime.jsx)("span", {
									className: Rows_module_css_default.searchResultSnippet,
									children: colored(result.snippet, result.matchRuns)
								})]
							})]
						}), result.matches !== void 0 && result.matches.length > 1 && (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							className: "dsh-sp-chevron",
							"aria-label": expanded ? t("searchPlus.collapse") : t("searchPlus.expand"),
							onClick: (e) => {
								e.stopPropagation();
								onToggleExpand();
							},
							children: `${expanded ? "▴" : "▾"} ${result.matches.length}`
						})]
					}),
		...
		*/
		...
		function SearchResults({ useSessions, open, jumpTo, workspaces, archivedSessionIds, query, remote, resultLimit, filters, t }) {
			const list = useSessions((s) => s);
			// [search-plus] any number of per-session dropdowns open at once.
			const [expandedSessions, setExpandedSessions] = (0, react.useState)(() => /* @__PURE__ */ new Set());
			const toggleExpanded = (id) => setExpandedSessions((previous) => {
			...
			const results = (0, react.useMemo)(() => deriveSearchResults(list, workspaces, query, archivedSessionIds, currentRemote, resultLimit, filters), [
				list,
				workspaces,
				query,
				archivedSessionIds,
					...
					children: [
						(0, react_jsx_runtime.jsx)("div", {
							...
							children: results.items.map((result) => (0, react_jsx_runtime.jsx)(SearchResultItem, {
								...
								currentId: list.current,
								onOpen: open,
								// [search-plus] jump wiring + matching-mode flags.
								onJump: jumpTo,
								filters,
								query,
		...
		*/
		function WorkspaceBrowser({ wide, expandSidebar, useSessions, useWorkspaces, useStore, actions, startSession, open, renameSession, forkSession, renameWorkspace, deleteWorkspace, insertWorkspaceBefore, archiveSession, insertSessionBefore, createWorkspace, searchSessions, searchResultLimit, useDirectoryFlow, renderSlot, t, jumpTo, clearJump, remarkJump, getJump, subscribeJump }) {
			...
			// observer sits on document.body: the scroll container is remounted
			// on every session switch, an element-level observer would go deaf.
			(0, react.useEffect)(() => {
				if (jump.target === null) return;
				let timer;
				const observer = new MutationObserver(() => {
					window.clearTimeout(timer);
					timer = window.setTimeout(() => {
						if (!searchPlusHighlight.hasMarks()) remarkJump();
					}, 300);
				});
			...
			// [search-plus] navigating away from the jumped session retires the
			// marks — but only on an EXTERNAL navigation: the jump itself moves
			// `current` to the target session, which must not count.
			const currentSessionId = useSessions((s) => s.current);
			const prevCurrentRef = (0, react.useRef)(currentSessionId);
			(0, react.useEffect)(() => {
			...
			(0, react.useEffect)(() => {
				...
				const timer = window.setTimeout(() => {
					// [search-plus] pass filters; the host returns richer items
					// ({sessionId, snippet, matchRuns, matches, hasMore}).
					searchSessions(normalizedQuery, searchFilters, controller.signal).then((result) => {
			...
			}, [normalizedQuery, searchSessions, searchFilters]);
		...
		// [search-plus] the official workspace dicts moved to ./locales.ts
		// (paradigm copy layer; content byte-preserved from the rc.6 baseline).
		const zh = searchPlusLocales.zh;
		const en = searchPlusLocales.en;
		//#endregion
		...
		*/
		function apply(ctx) {
			ctx.effect(() => ctx.locale.register(NS, {
				// [search-plus] official dicts + search-plus additions.
				zh: { ...zh, ...searchPlusZh },
				en: { ...en, ...searchPlusEn }
			}), "ui-workspace: dictionaries");
			// [search-plus] inject the filter/dropdown/mark stylesheet once.
			ensureSearchPlusCss();
			// [search-plus] content search goes through the private in-memory
			// index route (fast, filters, per-session hits); the official
			// ctx.sessions.search remains the fallback on transport failure.
			const searchSessions = async (query, filters, signal) => {
				const response = await fetch("/api/search-plus/query", {
					method: "POST",
				...
				if (!response.ok) throw new Error(`search-plus http ${response.status}`);
			...
			const browserInjected = () => ({
				...
				// [search-plus] open + locate + highlight one content hit.
				jumpTo: (target) => {
					setJumpState({ target });
					void searchPlusHighlight.jumpToMatch(ctx.sessions, target);
				},
				clearJump: () => {
					searchPlusHighlight.clearMarks();
					setJumpState({ target: null });
				},
				// [search-plus] re-mark the current target after React re-renders.
				remarkJump: () => {
					const target = getJumpState().target;
					if (target !== null) searchPlusHighlight.remark(ctx.sessions, target);
				},
				getJump: getJumpState,
				subscribeJump,
				searchSessions,
